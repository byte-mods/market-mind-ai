...
from marketmind.ml.causal.inference import get_causal_inference_engine
...
async def get_options_symbols():
    """Return all F&O tradable symbols (indices + equities)."""
    fetcher = get_options_fetcher()
...
async def fo_sentiment():
    ...
    def _get_chain(sym: str):
        """NSE first, then Kite fallback."""
        chain = fetcher.get_option_chain(sym)
        if chain and not chain.get('unavailable') and chain.get('calls'):
            return chain
        if kite and kite.is_connected:
            kite_chain = fetcher.get_option_chain_from_kite(sym, kite)
            if kite_chain and kite_chain.get('calls'):
                return kite_chain
        return chain

    def fetch():
        nifty = _get_chain('NIFTY')
        banknifty = _get_chain('BANKNIFTY')

        # If both chains are empty/unavailable, surface that clearly instead of
        # rendering a Neutral/50 gauge with zero max-pain and zero OI.
        nifty_unavailable = not nifty or nifty.get('unavailable') or not nifty.get('calls')
        banknifty_unavailable = not banknifty or banknifty.get('unavailable') or not banknifty.get('calls')
        if nifty_unavailable and banknifty_unavailable:
            return {
                'unavailable': True,
                'reason': 'NSE options data unavailable — markets may be closed (09:15–15:30 IST) or the feed is delayed.',
                'nifty_pcr': None,
                'banknifty_pcr': None,
                'avg_pcr': None,
                'nifty_max_pain': None,
                ...
                'nifty_put_oi': None,
                'source_nifty': 'nse',
                'source_banknifty': 'nse',
                'interpretation': None,
            }

        # Build values from available chains only; None for unavailable so the
        # frontend can show "—" instead of defaulting to neutral 1.0 / 0.
        nifty_pcr = nifty.get('pcr') if not nifty_unavailable else None
        bn_pcr = banknifty.get('pcr') if not banknifty_unavailable else None
        available_pcrs = [p for p in (nifty_pcr, bn_pcr) if p is not None]
        avg_pcr = round(sum(available_pcrs) / len(available_pcrs), 3) if available_pcrs else 1.0
        ...
        return {
            ...
            'avg_pcr': avg_pcr if available_pcrs else None,
            'nifty_max_pain': nifty.get('max_pain') if not nifty_unavailable else None,
            'banknifty_max_pain': banknifty.get('max_pain') if not banknifty_unavailable else None,
            'sentiment': sentiment if available_pcrs else None,
            'gauge': gauge if available_pcrs else None,
            'nifty_call_oi': nifty.get('total_call_oi') if not nifty_unavailable else None,
            'nifty_put_oi': nifty.get('total_put_oi') if not nifty_unavailable else None,
            'source_nifty': nifty.get('source', 'nse'),
            'source_banknifty': banknifty.get('source', 'nse'),
            'interpretation': (
    ...
    try:
        ...
        data = {
            'unavailable': True,
            'reason': 'NSE options data temporarily unavailable.',
            'nifty_pcr': None,
            'banknifty_pcr': None,
            'avg_pcr': None,
            'nifty_max_pain': None,
...
async def rl_signals():
    """
    Trained-model signals when available; otherwise fall back to persisted
    historical signals from SQLite so the history table shows correctly-shaped
    data (entry_price / exit_price / stop_loss / rationale fields).
    """
    signals = await _run(controller.get_rl_signals)
    if signals:
        return JSONResponse(_sanitize(signals))
    # Fallback: persisted historical signals with the field names the UI expects
    try:
        history = await _run(controller.db.get_recent_signals, limit=20)
        return JSONResponse(_sanitize(history))
    except Exception:
        return JSONResponse([])


def _resolve_rl_cp(sym, kite_prices, pf, df, kite=None):
    """Resolve the anchor price for an RL signal: Kite live → NSE live → Kite LTP → history.

    Pure helper (no I/O of its own beyond what `pf.get_stock_price` and the optional
    `kite.get_ltp` do). Returns `0.0` only if every layer fails; callers skip the
    symbol on a non-positive price.

    The chain is strictly ordered:
      1. `kite_prices[sym]['current_price']` — populated upstream by a single bulk
         `kite.get_ohlc` call. Most authoritative (real-time LTP from Kite REST).
      2. `pf.get_stock_price(sym)['current_price']` — NSE live quote, routed through
         the project's mandatory 5-min cache. Engaged when Kite is offline or didn't
         return this symbol. Exceptions are swallowed.
      3. `kite.get_ltp([f"NSE:{sym}"])` — targeted Kite LTP fallback. The bulk OHLC
         endpoint sometimes returns `last_price: 0` for symbols with degraded feeds;
         the dedicated LTP endpoint is more reliable. Only engaged when `kite` is
         passed and connected.
      4. `df['close'].iloc[-1]` — last cached daily close. Last-resort anchor when
         all live sources are unavailable.
    """
    cp = (kite_prices.get(sym) or {}).get('current_price') or 0
    if cp and cp > 0:
        return float(cp)
    try:
        nse_q = pf.get_stock_price(sym)
        if nse_q and nse_q.get('current_price') and nse_q['current_price'] > 0:
            return float(nse_q['current_price'])
    except Exception as e:
        logger.debug(f"NSE live fallback failed for {sym}: {e}")
    if kite is not None:
        try:
            if kite.is_connected:
                ltp_resp = kite.get_ltp([f"NSE:{sym}"])
                ltp_info = ltp_resp.get(f"NSE:{sym}", {}) if ltp_resp else {}
                ltp = float(ltp_info.get('last_price', 0)) if ltp_info else 0.0
                if ltp > 0:
                    return ltp
        except Exception as e:
            logger.debug(f"Kite LTP fallback failed for {sym}: {e}")
    try:
        hist = float(df['close'].iloc[-1])
        if hist > 0:
            return hist
    except Exception:
        pass
...
async def rl_multiframe(refresh: bool = False):
    """Multi-timeframe RL signals: intraday, swing, positional.

    Every call re-scans live (Kite REST is uncached; NSE goes through its
    own 5-min internal cache per project rules). The `refresh` query flag
    is accepted to let the UI cache-bust the HTTP layer (`?refresh=true&_=ts`)
    and signal user-initiated reloads, but server-side behaviour is the
    same in both modes — Entry/Target/SL always anchor on the freshest
    price available via Kite live LTP → NSE live → historical close.
    """
    _ = refresh  # accepted for cache-busting; when true, also force-refreshes candle data
    from marketmind.core.price_fetcher import get_price_fetcher
    pf = get_price_fetcher()
    ...
    def fetch():
        ...
        for sym in SYMBOLS:
            try:
                ...
                last = feat.iloc[-1]
                prev = feat.iloc[-2]
                cp = _resolve_rl_cp(sym, kite_prices, pf, df, kite)
                if cp <= 0:
                    continue
...
# ═════════════════════════════════════════════════════════════════════════════
# MULTI-ASSET (W4.2) — MCX commodities, forex, crypto, cross-asset correlations
# ═════════════════════════════════════════════════════════════════════════════

@app.get("/api/multiasset")
async def multiasset_dashboard():
    """Aggregate multi-asset data: commodities, forex, crypto."""
    maf = get_multiasset_fetcher()
    result = await _run(maf.get_all, controller.kite, controller.price_fetcher)
    return JSONResponse(_sanitize(result))


@app.get("/api/multiasset/commodities")
async def multiasset_commodities():
    """MCX commodity prices (gold, silver, crude oil, zinc) via Kite."""
    maf = get_multiasset_fetcher()
    result = await _run(maf.get_commodities, controller.kite)
    return JSONResponse(_sanitize(result))
...
@app.get("/api/multiasset/crypto")
async def multiasset_crypto():
    """BTC/INR + ETH/INR live prices via CoinGecko free tier."""
    maf = get_multiasset_fetcher()
    result = await _run(maf.get_crypto)
    return JSONResponse(_sanitize(result))


@app.get("/api/multiasset/correlations")
async def multiasset_correlations(windows: str = "30,90"):
    """Rolling cross-asset Pearson correlations (Nifty, Gold, BTC).
    Query: ?windows=30,90,60"""
    try:
    ...
    maf = get_multiasset_fetcher()
    result = await _run(maf.get_cross_asset_correlations,
                        controller.kite, controller.price_fetcher, win_list)
    return JSONResponse(_sanitize(result))
...
async def causal_nodes():
    """Return all nodes in the causal network with current values + parents."""
    try:
        engine = get_causal_inference_engine()
        nodes = await _run(engine.get_nodes)
        summary = await _run(engine.get_network_summary)
        return JSONResponse({"nodes": nodes, "summary": summary})
    except Exception as e:
        logger.error(f"Causal nodes error: {e}")
        return JSONResponse({"nodes": [], "summary": {}, "error": str(e)})


@app.post("/api/causal/whatif")
async def causal_whatif(req: _CausalWhatIfRequest):
    """Counterfactual estimate: ``what happens to <target> if I set <node>=<value>``.

    Body: ``{intervention: {repo_rate: 5.5, crude_oil: 70.0}, target: "banknifty"}``
    """
    try:
        engine = get_causal_inference_engine()
        result = await _run(engine.whatif, req.intervention, req.target)
        return JSONResponse(result)
    except ValueError as e:
        return JSONResponse({"error": str(e)}, status_code=400)
    except Exception as e:
        logger.error(f"Causal whatif error: {e}")
